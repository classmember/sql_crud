...
void Entity::create()
{
    ...
    if (sqlite3_exec(this->db,
		     query.c_str(),
		     NULL,
		     static_cast < void *>(this), error) == SQLITE_OK) {
	;			// success
    } else {
	cout << "id out of range" << endl;
    }
    // get new id from the database's autoincrement id
    string count_rows_query = "SELECT MAX(id) AS max_id " "FROM Entities;";
    if (sqlite3_exec(this->db,
	...
	cerr << "create error" << endl << error << endl;
    }
...
void Entity::update()
{
	...
	cerr << "update error" << endl << error << endl;
    }
}
...
void Entity::read()
{
    char **error = NULL;
    string query = "SELECT * "
	"FROM Entities " "WHERE id = '" + this->id + "';";
	...
	cerr << "read error" << endl << error << endl;
    }
}
...
void Entity::remove()
{
	...
	cerr << "remove error" << endl << error << endl;
    }
}
